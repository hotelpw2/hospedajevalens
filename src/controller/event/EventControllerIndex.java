...
public class EventControllerIndex extends HttpServlet {
	...
	public void doGet(HttpServletRequest req, HttpServletResponse res) throws IOException, ServletException {
		 ...
		 * name=req.getParameter("id"); String query = "select from " +
		 * Event.class.getName();
		 * 
		 * @SuppressWarnings("unchecked") List<Event> events = (List<Event>)
		 * pm.newQuery(query).execute(); Event elegido; String iddd=null;
		 * for(int i=0;i<events.size();i++) {
		 * iddd=events.get(i).getId().toString(); if(iddd.equals(name)) {
		 * elegido=events.get(i); req.setAttribute("events", elegido); //
		 * forward the request to the jsp RequestDispatcher dispatcher =
		 * getServletContext().getRequestDispatcher("/WEB-INF/Events/index.jsp")
		 * ; dispatcher.forward(req, res); try{ pm.makePersistent(elegido);
		 * }finally{ pm.close(); } } }
		 */
					...
					for (int i = 0; i < events.size(); i++) {
						...
						if (iddd.equals(name)) {
							...
							RequestDispatcher dispatcher = getServletContext()
									.getRequestDispatcher("/WEB-INF/Events/index.jsp");
