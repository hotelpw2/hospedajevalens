				...
				if (accesoRolePersonaEncontrada.equals(s1)) {
					...
					if (req.getParameter("action").equals("eventCreate")) {
						RequestDispatcher dispatcher = getServletContext()
								.getRequestDispatcher("/WEB-INF/Events/add.jsp");
						dispatcher.forward(req, res);
					} else if (req.getParameter("action").equals("eventCreateDo")) {
						...
						y = day + "/" + month + "/" + year;
						x = hora + " " + hora2;
						Event a = new Event((String) req.getParameter("name"), (String) req.getParameter("location"), x,
								y, Integer.parseInt(req.getParameter("capacity")), true, sdf.format(date));
						try {
							pm.makePersistent(a);
						} finally {
							pm.close();
					...
					if (req.getParameter("action").equals("eventCreate")) {
						...
						if (accesoRolePersonaEncontrada.equals(s1)) {
							...
							RequestDispatcher dispatcher = getServletContext()
									.getRequestDispatcher("/WEB-INF/Views/deny.jsp");
							dispatcher.forward(req, res);
						}
					} else if (req.getParameter("action").equals("eventCreateDo")) {
						// create the new event
