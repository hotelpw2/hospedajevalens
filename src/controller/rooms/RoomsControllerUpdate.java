...
public class RoomsControllerUpdate extends HttpServlet {

	public void doGet(HttpServletRequest req, HttpServletResponse resp) throws IOException, ServletException {
		PersistenceManager pm = PMF.get().getPersistenceManager();
		Key k = KeyFactory.createKey(Room.class.getSimpleName(), new Long(req.getParameter("id")).longValue());
		Room r = pm.getObjectById(Room.class, k);
		req.setAttribute("room", r);
		req.getRequestDispatcher("/WEB-INF/Rooms/update.jsp").forward(req, resp);
		pm.close();
	}
			...
			if (userExisteBaseDatos && estadoUser && estadoAcceso && rolEnBaseDatos) {
				...
				if (accesoRolePersonaEncontrada.equals(s1) || accesoRolePersonaEncontrada.equals(s3)) {
					...
					Key k = KeyFactory.createKey(Room.class.getSimpleName(),
							new Long(req.getParameter("id")).longValue());
					Room r = pm.getObjectById(Room.class, k);
